...
The BLAST (Basic Local Alignment Search Tool) module performs sequence similarity searches. It takes a single nucleotide or protein sequence (the **Query**) and compares it against a comprehensive database of sequences to find regions of local similarity.

The results are displayed as interactive cards, showing the most significant matches based on E-value, identity, and location.
...
| :--- | :--- | :--- |
| **Query Sequence** | Single nucleotide (DNA) or protein sequence (amino acid). | The sequence must be provided in raw text format (not FASTA, though the example implies raw sequence data). |
